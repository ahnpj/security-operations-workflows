...
(`memory-analysis-using-volatility`)</br>
Focuses on practical memory forensics using the Volatility Framework 2.6.1 command-line interface to identify operating system metadata, determine the correct memory profile, enumerate active processes, analyze process relationships, recover command-line execution artifacts, review network connections, and extract executables from memory. Demonstrates memory image identification using imageinfo, profile selection, KDBG interpretation, process enumeration with pslist, parent-child process analysis with pstree, command-line recovery with cmdline, network analysis using netscan, process extraction using procdump, and hash generation for memory-resident executables. Reinforces foundational memory forensics concepts including profile-based memory parsing, volatile artifact interpretation, process-focused investigation, runtime evidence analysis, suspicious process lineage identification, network artifact review, and preservation of memory-resident evidence for further forensic examination.

- **Memory Forensics and Process Analysis Using Volatility Workbench**</br>
(`memory-forensics-and-process-analysis-using-volatility`)</br>
Focuses on analyzing Windows memory images using Volatility Workbench and Volatility 3 plugins to identify operating system metadata, enumerate active processes, investigate process relationships, review process privileges, and examine command-line execution artifacts. Demonstrates practical examination of volatile memory evidence through memory image loading, process enumeration, Windows operating system identification, process tree analysis, privilege attribution, PID-based investigation, and command-line artifact recovery. Covers identification of PowerShell parent processes, privilege attribute analysis, process-focused memory examination, execution context review, and correlation of memory artifacts across multiple Volatility plugins. Reinforces foundational DFIR concepts including volatile evidence analysis, process lineage investigation, process capability interpretation, command-line artifact analysis, runtime evidence correlation, and host activity reconstruction using memory-resident artifacts.

- **Disk Image Forensics and Artifact Analysis Using Autopsy**</br>
(`disk-image-forensics-and-artifact-analysis-using-autopsy`)</br>
Focuses on practical disk image forensics using Autopsy to examine a forensic disk image and recover operating system information, hostname metadata, browser download artifacts, source URLs, Recent Documents artifacts, Windows shortcut (.lnk) evidence, virtual filesystem metadata, and local operating system account information. Demonstrates forensic case creation, evidence ingestion, ingest module configuration, artifact-based analysis, filesystem navigation, download reconstruction, shortcut artifact interpretation, directory metadata examination, and account activity review. Covers operating system identification, hostname attribution, web download analysis, URL recovery, recent file access reconstruction, virtual filesystem investigation, installed application directory review, and account timestamp analysis. Reinforces foundational DFIR concepts including disk image examination, artifact correlation, filesystem analysis, browser artifact analysis, shortcut artifact interpretation, user activity reconstruction, account attribution, and host-based evidence analysis.
