# Workflow Execution — Targeted Packet Capture and Traffic Filtering Using tcpdump

---

## Overview

This section establishes the operational goals of the workflow exeution, the packet-capture concepts being exercised, and how command-line traffic collection supports real-world network investigation and response scenarios.
...
This workflow execution write-up documents a structured, repeatable approach to capturing and analyzing network packets using `tcpdump` from a command-line environment. The primary focus is on performing targeted packet collection from specific network interfaces while controlling capture scope through packet limits, name-resolution suppression, and protocol-aware filtering.

Rather than capturing indiscriminately, the workflow emphasizes deliberate scoping of traffic using Berkeley Packet Filter (BPF) expressions to isolate packets by:

- host and direction (source and destination)
- port and service class
- protocol type
- packet length thresholds
- TCP flag state and connection behavior

The workflow also highlights two complementary analysis modes:

- **Live capture**, where packets are observed directly from an active interface in real time  
- **Offline analysis**, where saved `.pcap` files are replayed, filtered, and counted for repeatable inspection

Together, these approaches model how analysts collect evidence during live response while preserving artifacts for later correlation, escalation, or reporting.

A secondary operational focus is efficiency and accessibility. `tcpdump` is lightweight, fast, and commonly available by default on Linux systems, making it particularly useful in environments where:

- graphical tools are unavailable
- access is limited to SSH
- rapid triage is required with minimal host impact

This framing reflects real incident response conditions where analysts must rely on native tooling to understand network behavior quickly and accurately.
...
### Execution Stage 1 — Basic Packet Capture (Interface Validation + Bounded Capture)

**Objective:**
Understand how to perform basic packet captures using Tcpdump, specify which network interface to listen on, and save the captured data for later review.
...
#### Step 2 — Start a bounded capture on the selected interface
...
#### Operational notes captured from this stage
...
### Execution Stage 2 — Filtering Expressions (Host + Port + Protocol + Offline Review)

This stage focuses on narrowing capture scope to specific targets. Without filtering, packet output becomes unmanageable quickly, especially on active interfaces.

**Objective:**
Focus captures on specific hosts, ports, or protocols using Tcpdump filtering expressions. Without filters, packet captures can be massive and difficult to analyze.

#### Step 1 — Filter capture by host (domain)
...
#### Step 2 — Filter live capture by port (DNS example)
...
#### Step 3 — Filter live capture by protocol (ICMP example)
...
#### Step 3 (continued) — Combine filters using logical operators
...
#### Step 4 — Filter packets from an existing `.pcap` file (source filtering + counting)
...
#### Step 5 — Additional offline analysis practice (ICMP + ARP + DNS domain extraction)
...
##### Step 5-a — Count ICMP packets in `traffic.pcap`
...
##### Step 5-b — Identify the host requesting a MAC address (ARP pivot)
...
##### Step 5-c — Extract the first DNS query hostname from the capture
...
#### Operational notes from this stage
...
### Execution Stage 3 — Advanced Filtering (Packet Size, Bitwise Logic, Byte Offsets, and TCP Flags)
...
#### Step 1 — Filter packets by length (`greater` / `less`)
...
##### Step 1-a — Capture packets larger than a threshold on the default interface
...
##### Step 1-b — Capture packets larger than a threshold on a specified interface
...
##### Step 1-c — Capture packets larger than a threshold involving a specific host
...
##### Step 1-d — Capture large TCP packets involving a specific host (multi-condition filter)
...
#### Step 2 — Review binary operations used in protocol-level filtering
...
#### Step 3 — Filter using header byte offsets (`proto[expr:size]`)
...
##### Step 3-a — Combine bitwise header byte filters with host filters (multicast example)
...
#### Step 4 — TCP flag analysis (handshake and connection-state inference)
...
##### Step 4-a — Capture TCP packets with **only** SYN set (all other flags unset)
...
##### Step 4-b — Capture TCP packets with SYN set (SYN present, other flags allowed)
...
##### Step 4-c — Capture TCP packets with SYN **or** ACK set (handshake visibility)
...
##### Step 5 — Retrieve TCP flag information from a `.pcap` file (RST-only counting + large packet origin)
...
### Execution Stage 4 — Displaying Packets (Header and Payload Interpretation Modes)
...
**Objective:**
Focuse on customizing Tcpdump’s output to better interpret packet data. I learned how to display packet headers and payloads in multiple formats, including ASCII and hexadecimal.
...
#### Step 1 — Display basic packet information (sample from a capture file)
...
#### Step 2 — Display brief packet information (quick view)
...
#### Step 3 — Include MAC addresses and link-layer headers
...
#### Step 4 — Display packets as ASCII
...
#### Step 5 — Display packets in hexadecimal format
...
#### Step 6 — Display packets using both hex and ASCII simultaneously
...
#### Step 7 — Identify MAC address of a host that sent an ARP request
