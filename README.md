...
| **digital-forensics-and-evidence-analysis** | Digital forensics and evidence analysis workflows focused on acquiring, validating, examining, and interpreting digital evidence from storage media, memory, files, and forensic artifacts. | Workflows for forensic disk image analysis, file system identification, partition analysis, metadata examination, file carving, hash validation, memory acquisition, process dumping, browser artifact collection, evidence integrity verification, forensic imaging, and foundational DFIR investigation methodologies. |
