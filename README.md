...
| - **digital-forensics-and-evidence-analysis** | Digital forensics and evidence analysis workflows focused on acquiring, validating, examining, interpreting, and correlating digital evidence from storage media, memory, files, operating systems, browser artifacts, and forensic artifacts. Workflows include forensic disk image analysis, file system identification, partition analysis, metadata examination, file carving, hash validation, memory acquisition, memory image analysis, Volatility-based memory forensics, operating system profile identification, process enumeration, process tree analysis, command-line artifact recovery, network connection analysis, process dumping, browser artifact collection, browser history analysis, cached content analysis, download investigation, evidence integrity verification, forensic imaging, Windows artifact analysis, shortcut (.LNK) analysis, Prefetch analysis, Jump List analysis, Recycle Bin analysis, deleted-file metadata examination, SID attribution analysis, deletion timestamp analysis, execution evidence review, user activity reconstruction, artifact correlation, timeline development, and foundational DFIR investigation methodologies. | Workflows for identifying deleted files, recovering deleted-file metadata, attributing deleted activity to specific users, analyzing Security Identifiers (SIDs), reviewing deletion timestamps, correlating original file paths, validating deleted-file ownership, and reconstructing deleted-file activity using RBCmd, CSVQuickViewer, and Windows Recycle Bin artifacts. Additional workflows include memory image identification, Volatility profile selection, process enumeration, parent-child process analysis, command-line recovery, network connection investigation, process extraction, executable hashing, and volatile evidence analysis using Volatility Framework 2.6.1 and Linux command-line utilities. |
...
- **Memory Forensics and Process Analysis Using Volatility – Operational Execution**</br>
